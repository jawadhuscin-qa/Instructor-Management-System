describe('playlist_module Testcases', () => {
  ...
  it('playlist_room_scan', () => {
    ...
    cy.get('#react-tabs-4 > .ctab-item > .ctab-label').click();
    cy.log('user is navigated to location tab');
    waitForLoaderToDisappear();

    // open live chat dialog
    cy.get('.live-chat-icon > img').click();

    // navigation on candidate profile
    cy.get(':nth-child(3) > .Tooltip-Wrapper > .icons-box > .header-icons').click();
    cy.go('back');
    waitForLoaderToDisappear();

    // navigation on Question view
    cy.get(':nth-child(4) > .Tooltip-Wrapper > .icons-box > .header-icons').click();
  });
});
